...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.flash:
            #rearrange to (batch_size, sequence_length, num_heads, head_size_og), we currently have batch, heads, sequence_length, head_size
...
def profile_model():
    ...
    input_tensor = torch.randn(1, 3, 100, 224, 224,dtype=torch.bfloat16).to('cuda')  # Example input dimensions
    ...
    target = torch.randint(0, 1000, (1,)).to('cuda')  # Example target for classification
...
if __name__ == '__main__':
    # Run the profile model function
    profile_model()
