...
class Bottleneck(nn.Module):
    def __init__(self, in_channels, out_channels, expanded_channels, stride=1, use_se=False, kernel_size=3,nonlinearity=nn.Hardswish(),batchnorm=True):
        ...
        self.squeeze_excite = SEBlock(expanded_channels) if use_se else None 
...
class MobileNetLarge(nn.Module):
    ...
    #3x3 bottlenecks1 (3, ReLU): 112x112x16 -> 56x56x24
        self.block2 = nn.Sequential(
            Bottleneck(in_channels=16,out_channels=16,expanded_channels=16,stride=1,nonlinearity=nn.ReLU()),
            Bottleneck(in_channels=16,out_channels=24,expanded_channels=64,stride=2,nonlinearity=nn.ReLU()),
            Bottleneck(in_channels=24,out_channels=24,expanded_channels=72,stride=1,nonlinearity=nn.ReLU())
            )
    #5x5 bottlenecks1 (3, ReLU, squeeze-excite): 56x56x24 -> 28x28x40
        self.block3 = nn.Sequential(
            Bottleneck(in_channels=24,out_channels=40,expanded_channels=72,stride=2,use_se=True,kernel_size=5,nonlinearity=nn.ReLU()),
            Bottleneck(in_channels=40,out_channels=40,expanded_channels=120,stride=1,use_se=True,kernel_size=5,nonlinearity=nn.ReLU()),
            Bottleneck(in_channels=40,out_channels=40,expanded_channels=120,stride=1,use_se=True,kernel_size=5,nonlinearity=nn.ReLU())
            )
    #3x3 bottlenecks2 (6, h-swish, last two get squeeze-excite): 28x28x40 -> 14x14x112
        self.block4 = nn.Sequential(
            Bottleneck(in_channels=40,out_channels=80,expanded_channels=240,stride=2),
            Bottleneck(in_channels=80,out_channels=80,expanded_channels=240,stride=1),
            Bottleneck(in_channels=80,out_channels=80,expanded_channels=184,stride=1),
            Bottleneck(in_channels=80,out_channels=80,expanded_channels=184,stride=1),
            Bottleneck(in_channels=80,out_channels=112,expanded_channels=480,stride=1,use_se=True),
            Bottleneck(in_channels=112,out_channels=112,expanded_channels=672,stride=1,use_se=True)
            )
    #5x5 bottlenecks2 (3, h-swish, squeeze-excite): 14x14x112 -> 7x7x160
        self.block5 = nn.Sequential(
            Bottleneck(in_channels=112,out_channels=160,expanded_channels=672,stride=2,use_se=True,kernel_size=5),
            Bottleneck(in_channels=160,out_channels=160,expanded_channels=960,stride=1,use_se=True,kernel_size=5),
            Bottleneck(in_channels=160,out_channels=160,expanded_channels=960,stride=1,use_se=True,kernel_size=5)
            )
    #conv3d (h-swish), avg pool 7x7: 7x7x960 -> 1x1x960
        self.block6 = nn.Sequential(
            ...
            nn.BatchNorm3d(960),
            nn.Hardswish(),
            nn.AvgPool3d(kernel_size=(1,7,7),stride=1)
            )
    #LSTM: 1x1x960 ->
...
class MobileNetSmall(nn.Module):
    ...
    #conv3d (h-swish): 224x224x3 -> 112x112x16
        self.block1 = nn.Sequential(
            nn.Conv3d(in_channels=3,out_channels=16,kernel_size=(1,3,3),stride=2,padding=1),
            nn.BatchNorm3d(16),
            nn.Hardswish()
            )
    #3x3 bottlenecks (3, ReLU, first gets squeeze-excite): 112x112x16 -> 28x28x24
        self.block2 = nn.Sequential(
            Bottleneck(in_channels=16,out_channels=16,expanded_channels=16,stride=2,use_se=True,nonlinearity=nn.ReLU()),
            Bottleneck(in_channels=16,out_channels=24,expanded_channels=72,stride=2,nonlinearity=nn.ReLU()),
            Bottleneck(in_channels=24,out_channels=24,expanded_channels=88,stride=1,nonlinearity=nn.ReLU())
            )
    #5x5 bottlenecks (8, h-swish, squeeze-excite): 28x28x24 -> 7x7x96
        self.block3 = nn.Sequential(
            Bottleneck(in_channels=24,out_channels=40,expanded_channels=96,stride=2,use_se=True,kernel_size=5),
            Bottleneck(in_channels=40,out_channels=40,expanded_channels=240,stride=1,use_se=True,kernel_size=5),
            Bottleneck(in_channels=40,out_channels=40,expanded_channels=240,stride=1,use_se=True,kernel_size=5),
            Bottleneck(in_channels=40,out_channels=48,expanded_channels=120,stride=1,use_se=True,kernel_size=5),
            Bottleneck(in_channels=48,out_channels=48,expanded_channels=144,stride=1,use_se=True,kernel_size=5),
            Bottleneck(in_channels=48,out_channels=96,expanded_channels=288,stride=2,use_se=True,kernel_size=5),
            Bottleneck(in_channels=96,out_channels=96,expanded_channels=576,stride=1,use_se=True,kernel_size=5),
            Bottleneck(in_channels=96,out_channels=96,expanded_channels=576,stride=1,use_se=True,kernel_size=5)
            )
    #conv2d (h-swish), avg pool 7x7: 7x7x96 -> 1x1x576
        self.block4 = nn.Sequential(
            nn.Conv3d(in_channels=96,out_channels=576,kernel_size=1,stride=1,padding=0),
            SEBlock(channels=576),
            nn.BatchNorm3d(576),
            nn.Hardswish(),
            nn.AvgPool3d(kernel_size=(1,7,7),stride=1)
            )
    #LSTM: 1x1x576 ->
        self.lstm = nn.LSTM(input_size=576,hidden_size=32,num_layers=5,batch_first=True)
...
#MobileNetV3-Small 2d with lstm
...
class MobileNetLargeNoLSTM(nn.Module):
    def __init__(self,num_classes=2):
        super().__init__()
...
class MobileNetSmallNoLSTM(nn.Module):
    def __init__(self,num_classes=2):
        super().__init__()
    ...
    #conv3d (h-swish): 224x224x3 -> 112x112x16
        self.block1 = nn.Sequential(
            nn.Conv3d(in_channels=3,out_channels=16,kernel_size=(3,3,3),stride=2,padding=1),
            nn.BatchNorm3d(16),
            nn.Hardswish()
            )
